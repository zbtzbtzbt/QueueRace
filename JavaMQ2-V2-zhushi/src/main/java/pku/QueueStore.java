...
public abstract class QueueStore {
     ...
     * put函数有两个参数
     * 把一条消息写入一个队列；
     * 这个接口需要是[线程安全]的，也即评测程序会[并发]调用该接口进行put；
     ...
     */
    //一个byte一个字节,故message长度从64到1k不等
    abstract void put(String queueName, byte[] message);

    /**
     * get函数有三个参数
     * 从一个队列中读出一批消息，读出的消息要按照发送顺序来；
     * 这个接口需要是[线程安全]的，也即评测程序会并发调用该接口进行get；
     * 返回的Collection会被[并发读]，但不涉及写，因此只需要是线程读安全就可以了；
     * @param queueName 代表队列的名字
     * @param offset 代表消息的在这个队列中的[起始消息索引]
